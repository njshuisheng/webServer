...
function list() {
  axios.post('/xkgw/qt/getInformationBybutton', { key: 4 }).then((res) => {
...
function del(r) {
  ...
  ElMessageBox.confirm('删除后不可恢复，确认删除吗?', '确认', {
    ...
    axios.post('/xkgw/ht/delete', { key: 4, id: r.id }).then((res) => {
      if (res.retCode === 0) {
...
function confirm() {
  ...
  axios
    .post('/xkgw/ht/save', Object.assign(data, form, { key: 5, type: 6 }))
    .then((res) => {
      if (res.retCode === 0) {
